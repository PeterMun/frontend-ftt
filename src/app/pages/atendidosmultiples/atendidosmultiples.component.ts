...
export class AtendidosmultiplesComponent implements OnInit {
  ...
  leeratendidosmultiples(){
    this.serviceService.getatendidosmultiples().subscribe((servicio: any) => {
...
generarPdf(action = 'open', pdf: number) {
  ...
  if (pdf === 1) {
    documentDefinition = this.getDocumentatendidosmultiples();
  }
...
getDocumentatendidosmultiples(){
  ...
  return {
    ...
    content: [
      ...
      {
        style: 'subtitulos',
        text: 'Reporte - Atendidos Multiples'
      },
      this.atendidosmult(this.servicio)
    ],
    styles: {
